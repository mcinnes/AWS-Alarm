...
foreach ($selectedArray as $value) {
  switch ($value) {
    case "0":
        ...
        break;
    case "1":
      $wordsToSpeak = $wordsToSpeak . getWeatherString($location);
        break;
    case "2":
...
$speech = [
  ...
  'Text' => '
    <speak>Good morning '. $firstName .', <break strength="strong"/> Today is <say-as interpret-as="date">'.$todayDate.'</say-as>. '.$wordsToSpeak.'</speak>',
...
$file = ParseFile::createFromFile($localFilePath, "matt.mp3");
$file->save();
